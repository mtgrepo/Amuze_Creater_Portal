...
interface ProfileWalletProps {
  data?: Partial<LoginCreatorResponse["acount"]>;
  wallet: Partial<LoginCreatorResponse["wallets"]>;
}

export default function ProfileWalletComponent({
  data,
  wallet,
}: ProfileWalletProps) {
  if (!data)
    ...
    <div className=" space-y-12 w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-card shadow-sm">
          ...
          <div className="p-8 md:col-span-2 bg-slate-50/50 dark:bg-slate-900/20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-12">
              {/* <DataPoint
                label="Provider"
                value={paymentData?.payment_info.payment_type.name}
                icon={<CreditCard className="w-3.5 h-3.5" />}
              /> */}
              <DataPoint
                label="Account Holder"
                value={paymentData?.payment_info?.account_holder_name}
                icon={<User className="w-3.5 h-3.5" />}
              />
              <DataPoint
                label="Account Number"
                value={paymentData?.payment_info?.account_no}
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {paymentData?.profit_percents &&
          paymentData?.profit_percents.length > 0 ? (
            paymentData.profit_percents.map((item) => (
              <div
                key={item.id}
            ...
            <div className="col-span-full py-12 text-center border border-dashed border-slate-300 dark:border-slate-700 rounded-xl">
              <p className="text-slate-500 text-sm">
                No revenue sharing configurations found.
              </p>
            </div>
          )}
        </div>
