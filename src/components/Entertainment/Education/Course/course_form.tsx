...
export default function CourseForm({
  ...
  const { textUpdateMutation, isPending: isUpdatingText } =
    useTextUpdateCommand();
  const { thumbnailUpdateMutation, isUpdatingThumbnail } =
    useThumbnailUpdateCommand();
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      if (mode === "add") {
        ...
        const isThumbnailUpdated = values.thumbnail instanceof File;
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-2xl font-bold">
                  {mode === "add"
                    ? t("course_form.create_title")
                    : t("course_form.update_title")}
                </h2>
