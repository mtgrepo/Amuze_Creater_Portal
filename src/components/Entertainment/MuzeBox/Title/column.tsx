...
export default function MuzeBoxColumn() {
  ...
  const columns: ColumnDef<MuzeBoxDetailsResponse>[] = [
    ...
    {
      accessorFn: (row) => ({
        is_published: row.is_publish,
        approve_status: row.approve_status,
      }),
      id: "status",
      header: t("status"),
      cell: ({ row }) => {
        const is_published = row.original.is_publish as boolean;
        const approve_status = row.original.approve_status as boolean;
        if (approve_status && is_published) {
          return (
            <IconWithTooltip
        ...
        } else if (approve_status && !is_published) {
          return (
            <IconWithTooltip
